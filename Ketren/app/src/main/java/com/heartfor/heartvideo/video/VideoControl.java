...
public class VideoControl extends HeartVideoParentControl implements View.OnClickListener, SeekBar.OnSeekBarChangeListener{
    ...
    public void setInfo(HeartVideoInfo info) {
        ...
        Picasso.with(context).load(info.getImagePath()).into(placeiv);
    }
    ...
    public void onPlayStateChanged(int currStatus) {
        switch (currStatus) {
            ...
            case PlayerStatus.STATE_PREPARING:
                ...
                control_loading_tv.setText("يۈكلىنىۋاتىدۇ…");
                break;
            case PlayerStatus.STATE_PREPARED:
                ...
                control_center_play.setVisibility(GONE);
                LoadingDialogShow(false);
                control_start_btn.setImageResource(R.drawable.icon_video_play);
            ...
            case PlayerStatus.STATE_PLAYING:
                ...
                control_center_play.setVisibility(GONE);
                LoadingDialogShow(false);
                control_start_btn.setImageResource(R.drawable.icon_video_pause);
            ...
            case PlayerStatus.STATE_ERROR:
                ...
                control_center_play.setVisibility(VISIBLE);
                LoadingDialogShow(true);
                control_loading_tv.setText("قويۇلمىدى…");
            ...
            case PlayerStatus.STATE_CHANGE_LINE:
                //切换视频源
                placeiv.setVisibility(GONE);
                Toast.makeText(getContext(), "ئ‍المىشىۋاتىدۇ", Toast.LENGTH_SHORT).show();
                upHandler.removeCallbacks(upRunable);
                upHandler.removeCallbacks(topAndbottomRunable);
                break;
        }
    ...
    public void onPlayModeChanged(int mCurrentMode) {
        if (mCurrentMode == PlayerStatus.MODE_FULL_SCREEN) {
            ...
            control_full.setImageResource(R.drawable.iconfont_exit);
            ...
            control_full.setImageResource(R.drawable.iconfont_enter_32);
    ...
    private BroadcastReceiver mBatterReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (status == BatteryManager.BATTERY_STATUS_CHARGING) {
                // 充电中
                control_title_battery.setImageResource(R.drawable.battery_charging);
            } else if (status == BatteryManager.BATTERY_STATUS_FULL) {
                // 充电完成
                control_title_battery.setImageResource(R.drawable.battery_full);
            } else {
                int level = intent.getIntExtra(BatteryManager.EXTRA_LEVEL, 0);
                int scale = intent.getIntExtra(BatteryManager.EXTRA_SCALE, 0);
                int percentage = (int) (((float) level / scale) * 100);
                if (percentage <= 10) {
                    control_title_battery.setImageResource(R.drawable.battery_10);
                } else if (percentage <= 20) {
                    control_title_battery.setImageResource(R.drawable.battery_20);
                } else if (percentage <= 50) {
                    control_title_battery.setImageResource(R.drawable.battery_50);
                } else if (percentage <= 80) {
                    control_title_battery.setImageResource(R.drawable.battery_80);
                } else if (percentage <= 100) {
                    control_title_battery.setImageResource(R.drawable.battery_100);
                }
            }
        }
    };
